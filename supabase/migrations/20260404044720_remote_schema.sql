...
COMMENT ON TYPE "public"."agent_type" IS 'types of AI agents available for conversations';
...
ALTER TYPE "public"."ai_run_status" OWNER TO "postgres";


COMMENT ON TYPE "public"."ai_run_status" IS 'status of an AI agent execution run';
...
COMMENT ON TYPE "public"."conflict_type" IS 'types of conflicts detected in trips';
...
COMMENT ON TYPE "public"."conversation_status" IS 'status of a conversation session';
...
COMMENT ON TYPE "public"."message_role" IS 'role of the message sender in the conversation';
...
COMMENT ON TYPE "public"."resolution_status" IS 'status of conflict resolution';
...
CREATE OR REPLACE FUNCTION "public"."broadcast_agent_jobs_changes"() RETURNS "trigger"
...
ALTER FUNCTION "public"."broadcast_agent_jobs_changes"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."broadcast_messages_changes"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."broadcast_messages_changes"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."broadcast_proactive_suggestions_changes"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."broadcast_proactive_suggestions_changes"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."broadcast_suggestions_changes"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."broadcast_suggestions_changes"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."broadcast_trip_items_changes"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."broadcast_trip_items_changes"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."broadcast_trips_changes"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."broadcast_trips_changes"() OWNER TO "postgres";
...
COMMENT ON FUNCTION "public"."calculate_distance"("lat1" numeric, "lng1" numeric, "lat2" numeric, "lng2" numeric) IS 'calculate distance in meters between two lat/lng coordinates using PostGIS';



CREATE OR REPLACE FUNCTION "public"."claim_agent_job"("p_worker_id" "text", "p_job_types" "text"[] DEFAULT NULL::"text"[]) RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
    v_job_id UUID;
BEGIN
    -- Atomically select and lock a queued job
...
ALTER FUNCTION "public"."claim_agent_job"("p_worker_id" "text", "p_job_types" "text"[]) OWNER TO "postgres";


COMMENT ON FUNCTION "public"."claim_agent_job"("p_worker_id" "text", "p_job_types" "text"[]) IS 'Atomically claim a queued job for processing';



CREATE OR REPLACE FUNCTION "public"."cleanup_expired_agent_jobs"() RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."cleanup_expired_agent_jobs"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."complete_agent_job"("p_job_id" "uuid", "p_result" "jsonb") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."complete_agent_job"("p_job_id" "uuid", "p_result" "jsonb") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."complete_agent_job"("p_job_id" "uuid", "p_result" "jsonb") IS 'Mark a job as completed with results';



CREATE OR REPLACE FUNCTION "public"."fail_agent_job"("p_job_id" "uuid", "p_error" "text") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."fail_agent_job"("p_job_id" "uuid", "p_error" "text") OWNER TO "postgres";


COMMENT ON FUNCTION "public"."fail_agent_job"("p_job_id" "uuid", "p_error" "text") IS 'Mark a job as failed, handles retry logic';
...
BEGIN
    IF NEW.confirmation_code IS NULL THEN
        NEW.confirmation_code := 'ILM-' || UPPER(SUBSTRING(md5(random()::text) FROM 1 FOR 8));
    END IF;
    RETURN NEW;
END;
$$;
...
COMMENT ON FUNCTION "public"."generate_confirmation_code"() IS 'Generate an 8-character uppercase confirmation code. Secured with SET search_path = empty string to prevent SQL injection.';
...
EXCEPTION
  WHEN unique_violation THEN
    -- Profile already exists (email duplicate), return new user anyway
    RETURN new;
  WHEN others THEN
...
COMMENT ON FUNCTION "public"."handle_new_user"() IS 'Trigger function to auto-create profile when a new user signs up.
   Uses SECURITY DEFINER to bypass RLS during signup.
   Fixed in migration 20260119185340.';
...
CREATE OR REPLACE FUNCTION "public"."realtime_broadcast_agent_jobs"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
  ...
  v_payload jsonb;
begin
  v_topic := 'job:' || coalesce(new.id, old.id)::text || ':status';

  -- safe payload for UI: no input_json, result_json, error, or locked_by
  v_payload := jsonb_build_object(
    'id', coalesce(new.id, old.id),
    'status', coalesce(new.status, old.status),
    'progress', coalesce(new.progress, old.progress),
    'current_step', coalesce(new.current_step, old.current_step),
    'total_steps', coalesce(new.total_steps, old.total_steps)
  );
...
ALTER FUNCTION "public"."realtime_broadcast_agent_jobs"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."realtime_broadcast_agent_jobs"() IS 'broadcasts job status/progress to job-scoped realtime topic for live progress UI; payload excludes secrets';



CREATE OR REPLACE FUNCTION "public"."realtime_broadcast_messages"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  v_topic := 'conversation:' || v_conversation_id::text || ':messages';
...
ALTER FUNCTION "public"."realtime_broadcast_messages"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."realtime_broadcast_messages"() IS 'broadcasts message insert/update/delete to conversation-scoped realtime topic for live chat';



CREATE OR REPLACE FUNCTION "public"."realtime_broadcast_trip_items"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  v_topic := 'trip:' || v_trip_id::text || ':items';
...
ALTER FUNCTION "public"."realtime_broadcast_trip_items"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."realtime_broadcast_trip_items"() IS 'broadcasts trip_item changes to trip-scoped realtime topic for live itinerary';



CREATE OR REPLACE FUNCTION "public"."realtime_broadcast_trips"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
declare
...
begin
  ...
  v_topic := 'trip:' || v_trip_id::text || ':meta';
...
ALTER FUNCTION "public"."realtime_broadcast_trips"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."realtime_broadcast_trips"() IS 'broadcasts trip meta changes to trip-scoped realtime topic for live trip header';



CREATE OR REPLACE FUNCTION "public"."release_stale_agent_job_locks"("p_stale_threshold" interval DEFAULT '00:10:00'::interval) RETURNS integer
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
DECLARE
...
ALTER FUNCTION "public"."release_stale_agent_job_locks"("p_stale_threshold" interval) OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_agent_job_progress"("p_job_id" "uuid", "p_progress" integer, "p_current_step" "text" DEFAULT NULL::"text") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
...
ALTER FUNCTION "public"."update_agent_job_progress"("p_job_id" "uuid", "p_progress" integer, "p_current_step" "text") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_collection_count"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    SET "search_path" TO ''
    AS $$
begin
...
ALTER FUNCTION "public"."update_collection_count"() OWNER TO "postgres";


COMMENT ON FUNCTION "public"."update_collection_count"() IS 'trigger function to update collection item count when saved places are added or removed';
...
COMMENT ON FUNCTION "public"."update_conversation_stats"() IS 'trigger function to update conversation statistics when a new message is inserted';
...
COMMENT ON FUNCTION "public"."update_updated_at"() IS 'trigger function to automatically update updated_at timestamp on row updates';
...
CREATE TABLE IF NOT EXISTS "public"."agent_jobs" (
    ...
    "locked_by" "text",
    CONSTRAINT "agent_jobs_progress_check" CHECK ((("progress" >= 0) AND ("progress" <= 100))),
    CONSTRAINT "agent_jobs_status_check" CHECK (("status" = ANY (ARRAY['queued'::"text", 'running'::"text", 'completed'::"text", 'failed'::"text", 'cancelled'::"text"]))),
    CONSTRAINT "agent_jobs_type_check" CHECK (("type" = ANY (ARRAY['trip_wizard'::"text", 'complex_trip_planning'::"text", 'deep_research'::"text", 'pitch_deck_generate'::"text", 'document_analysis'::"text", 'batch_image_generation'::"text"])))
);
...
COMMENT ON TABLE "public"."agent_jobs" IS 'Queue for heavy AI operations processed by worker service';



COMMENT ON COLUMN "public"."agent_jobs"."type" IS 'Job type: trip_wizard, complex_trip_planning, deep_research, pitch_deck_generate, document_analysis, batch_image_generation';



COMMENT ON COLUMN "public"."agent_jobs"."status" IS 'Job status: queued, running, completed, failed, cancelled';



COMMENT ON COLUMN "public"."agent_jobs"."progress" IS 'Progress percentage (0-100)';



COMMENT ON COLUMN "public"."agent_jobs"."run_after" IS 'Earliest time job can be picked up (for retry backoff)';



COMMENT ON COLUMN "public"."agent_jobs"."locked_by" IS 'Worker instance ID that claimed the job';
...
CREATE TABLE IF NOT EXISTS "public"."ai_context" (
    ...
    "entities" "jsonb" DEFAULT '{"dates": [], "people": [], "places": [], "constraints": {}, "preferences": {}}'::"jsonb",
    "current_intent" "text",
...
COMMENT ON TABLE "public"."ai_context" IS 'persistent context for AI conversations. stores session, user, and global context layers with tracked entities and conversation state.';
...
CREATE TABLE IF NOT EXISTS "public"."ai_runs" (
    ...
    "status" "public"."ai_run_status" DEFAULT 'pending'::"public"."ai_run_status",
    "error_message" "text",
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "completed_at" timestamp with time zone,
    CONSTRAINT "valid_cost" CHECK ((("estimated_cost_usd" >= (0)::numeric) OR ("estimated_cost_usd" IS NULL))),
...
COMMENT ON TABLE "public"."ai_runs" IS 'track AI agent execution for analytics and debugging. stores performance metrics, token usage, and cost tracking.';
...
CREATE TABLE IF NOT EXISTS "public"."apartments" (
    ...
    "building_amenities" "text"[] DEFAULT '{}'::"text"[],
    ...
    "freshness_status" "text" DEFAULT 'unconfirmed'::"text",
    "last_checked_at" timestamp with time zone,
    "raw_amenities" "jsonb" DEFAULT '{}'::"jsonb",
    CONSTRAINT "apartments_freshness_status_check" CHECK (("freshness_status" = ANY (ARRAY['active'::"text", 'unconfirmed'::"text", 'stale'::"text"]))),
    CONSTRAINT "apartments_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'inactive'::"text", 'booked'::"text", 'pending'::"text"])))
);
...
COMMENT ON TABLE "public"."apartments" IS 'Apartment and housing rental listings for short-term and long-term stays';



COMMENT ON COLUMN "public"."apartments"."source_url" IS 'canonical listing url at external source';



COMMENT ON COLUMN "public"."apartments"."source_listing_id" IS 'id/slug of listing at external source';



COMMENT ON COLUMN "public"."apartments"."freshness_status" IS 'active=confirmed, unconfirmed=needs recheck, stale=removed or unavailable';



COMMENT ON COLUMN "public"."apartments"."last_checked_at" IS 'last verification run timestamp';



COMMENT ON COLUMN "public"."apartments"."raw_amenities" IS 'raw amenity list from source';
...
COMMENT ON TABLE "public"."bookings" IS 'Unified bookings table for all reservation types: apartments, cars, restaurants, events';
...
CREATE TABLE IF NOT EXISTS "public"."budget_tracking" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "valid_budget" CHECK (("total_budget" >= (0)::numeric)),
    CONSTRAINT "valid_pending" CHECK (("total_pending" >= (0)::numeric)),
    CONSTRAINT "valid_spent" CHECK (("total_spent" >= (0)::numeric)),
    CONSTRAINT "valid_threshold" CHECK ((("alert_threshold" >= (0)::numeric) AND ("alert_threshold" <= (1)::numeric)))
);


ALTER TABLE "public"."budget_tracking" OWNER TO "postgres";


COMMENT ON TABLE "public"."budget_tracking" IS 'real-time budget monitoring per trip. tracks total budget, spent, pending amounts, and category breakdowns with AI recommendations.';
...
CREATE TABLE IF NOT EXISTS "public"."car_rentals" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "car_rentals_fuel_type_check" CHECK (("fuel_type" = ANY (ARRAY['gasoline'::"text", 'diesel'::"text", 'electric'::"text", 'hybrid'::"text"]))),
    CONSTRAINT "car_rentals_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'inactive'::"text", 'rented'::"text", 'maintenance'::"text"]))),
    CONSTRAINT "car_rentals_transmission_check" CHECK (("transmission" = ANY (ARRAY['automatic'::"text", 'manual'::"text"]))),
    CONSTRAINT "car_rentals_vehicle_type_check" CHECK (("vehicle_type" = ANY (ARRAY['sedan'::"text", 'suv'::"text", 'compact'::"text", 'van'::"text", 'luxury'::"text", 'pickup'::"text", 'convertible'::"text", 'minivan'::"text"])))
);
...
COMMENT ON TABLE "public"."car_rentals" IS 'Vehicle rental listings for cars, SUVs, and other vehicles';
...
COMMENT ON TABLE "public"."collections" IS 'user-created collections of places. supports sharing and organization of saved locations.';
...
CREATE TABLE IF NOT EXISTS "public"."conflict_resolutions" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "conflict_resolutions_resolved_by_check" CHECK (("resolved_by" = ANY (ARRAY['ai'::"text", 'user'::"text"]))),
    CONSTRAINT "conflict_resolutions_severity_check" CHECK ((("severity" >= 1) AND ("severity" <= 10)))
);
...
COMMENT ON TABLE "public"."conflict_resolutions" IS 'track detected conflicts and their resolutions in trips. supports automatic and user-driven conflict resolution.';
...
COMMENT ON TABLE "public"."conversations" IS 'AI chat sessions with different agents. supports multiple conversation types and status tracking.';
...
CREATE TABLE IF NOT EXISTS "public"."events" (
    ...
    CONSTRAINT "events_source_check" CHECK (("source" = ANY (ARRAY['google'::"text", 'ticketmaster'::"text", 'manual'::"text", 'ai_generated'::"text"]))),
    CONSTRAINT "valid_event_times" CHECK ((("event_end_time" IS NULL) OR ("event_start_time" <= "event_end_time"))),
    CONSTRAINT "valid_ticket_prices" CHECK ((("ticket_price_min" IS NULL) OR ("ticket_price_max" IS NULL) OR ("ticket_price_min" <= "ticket_price_max")))
);
...
COMMENT ON TABLE "public"."events" IS 'events table for concerts, festivals, shows, and other time-based activities. includes event-specific fields like timing and ticketing.';
...
COMMENT ON TABLE "public"."messages" IS 'individual messages in AI conversations. tracks role, content, AI metadata, and token usage.';
...
CREATE TABLE IF NOT EXISTS "public"."proactive_suggestions" (
    ...
    "metadata" "jsonb" DEFAULT '{}'::"jsonb",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "proactive_suggestions_confidence_score_check" CHECK ((("confidence_score" >= (0)::numeric) AND ("confidence_score" <= (1)::numeric))),
    CONSTRAINT "proactive_suggestions_priority_check" CHECK ((("priority" >= 1) AND ("priority" <= 10))),
    CONSTRAINT "proactive_suggestions_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'shown'::"text", 'accepted'::"text", 'rejected'::"text", 'expired'::"text"]))),
    CONSTRAINT "proactive_suggestions_type_check" CHECK (("type" = ANY (ARRAY['add_activity'::"text", 'book_restaurant'::"text", 'budget_alert'::"text", 'local_event'::"text", 'time_optimization'::"text", 'weather_alert'::"text", 'price_drop'::"text", 'cancellation_notice'::"text"])))
);
...
COMMENT ON TABLE "public"."proactive_suggestions" IS 'AI-generated proactive suggestions for users';
...
CREATE TABLE IF NOT EXISTS "public"."profiles" (
    ...
    "notification_preferences" "jsonb" DEFAULT '{"sms": false, "push": true, "email": true, "trip_updates": true, "ai_suggestions": true}'::"jsonb",
...
COMMENT ON TABLE "public"."profiles" IS 'User profiles. In development, FK to auth.users dropped for seed data.';
...
CREATE TABLE IF NOT EXISTS "public"."rental_freshness_log" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "rental_freshness_log_status_check" CHECK (("status" = ANY (ARRAY['active'::"text", 'unconfirmed'::"text", 'stale'::"text"])))
);


ALTER TABLE "public"."rental_freshness_log" OWNER TO "postgres";
...
ALTER TABLE "public"."rental_listing_images" OWNER TO "postgres";
...
ALTER TABLE "public"."rental_listing_sources" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."rental_search_sessions" (
    ...
    "result_ids" "uuid"[] DEFAULT '{}'::"uuid"[],
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL
);


ALTER TABLE "public"."rental_search_sessions" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."rentals" (
    ...
    "website" "text",
    "email" "text",
    "rental_features" "text"[] DEFAULT '{}'::"text"[],
    ...
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "created_by" "uuid",
    CONSTRAINT "rentals_rating_check" CHECK ((("rating" >= (0)::numeric) AND ("rating" <= (5)::numeric))),
    CONSTRAINT "rentals_source_check" CHECK (("source" = ANY (ARRAY['manual'::"text", 'ai_generated'::"text"]))),
    CONSTRAINT "valid_availability" CHECK ((("availability_end" IS NULL) OR ("availability_start" IS NULL) OR ("availability_start" <= "availability_end"))),
    CONSTRAINT "valid_daily_rate" CHECK ((("daily_rate" IS NULL) OR ("daily_rate" >= (0)::numeric))),
    CONSTRAINT "valid_hourly_rate" CHECK ((("hourly_rate" IS NULL) OR ("hourly_rate" >= (0)::numeric))),
    CONSTRAINT "valid_weekly_rate" CHECK ((("weekly_rate" IS NULL) OR ("weekly_rate" >= (0)::numeric)))
);
...
COMMENT ON TABLE "public"."rentals" IS 'rentals table for vehicles, apartments, equipment, and other rentable items. includes rental-specific fields like rates and availability.';
...
CREATE TABLE IF NOT EXISTS "public"."restaurants" (
    ...
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "created_by" "uuid",
    CONSTRAINT "restaurants_price_level_check" CHECK ((("price_level" >= 1) AND ("price_level" <= 4))),
    CONSTRAINT "restaurants_rating_check" CHECK ((("rating" >= (0)::numeric) AND ("rating" <= (5)::numeric))),
    CONSTRAINT "restaurants_source_check" CHECK (("source" = ANY (ARRAY['google'::"text", 'yelp'::"text", 'manual'::"text", 'ai_generated'::"text"])))
);
...
COMMENT ON TABLE "public"."restaurants" IS 'restaurants table for dining establishments. includes restaurant-specific fields like cuisine types, price level, and hours of operation.';
...
CREATE TABLE IF NOT EXISTS "public"."saved_places" (
    ...
    "trip_id" "uuid",
    CONSTRAINT "saved_places_location_type_check" CHECK (("location_type" = ANY (ARRAY['event'::"text", 'restaurant'::"text", 'rental'::"text", 'poi'::"text"]))),
...
COMMENT ON TABLE "public"."saved_places" IS 'user saved and favorited locations. supports organization into collections and tagging.';
...
CREATE TABLE IF NOT EXISTS "public"."tourist_destinations" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    "google_place_id" "text",
    "external_id" "text",
    "source" "text" DEFAULT 'manual'::"text" NOT NULL,
    ...
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "tour_operator" "text",
    "tour_operator_website" "text",
    "tour_inclusions" "text"[] DEFAULT '{}'::"text"[],
    "tour_exclusions" "text"[] DEFAULT '{}'::"text"[],
    "tour_duration_hours" numeric(4,2),
    "tour_duration_text" "text",
    "pickup_location" "text",
    "pickup_location_lat" numeric(10,8),
    "pickup_location_lng" numeric(11,8),
    "departure_times" "text"[] DEFAULT '{}'::"text"[],
    "booking_url" "text",
    ...
    "age_min" integer,
    "age_max" integer,
    "languages_available" "text"[] DEFAULT '{}'::"text"[],
    ...
    "audio_guide_languages" "text"[] DEFAULT '{}'::"text"[],
    "guided_tour_available" boolean DEFAULT false,
    "self_guided" boolean DEFAULT true,
    "parking_available" boolean,
    "parking_fee" numeric(10,2),
    "public_transport_access" "text",
    "nearby_attractions" "uuid"[] DEFAULT '{}'::"uuid"[],
    "related_destinations" "uuid"[] DEFAULT '{}'::"uuid"[],
    "peak_season" "text",
    ...
    CONSTRAINT "locations_source_check" CHECK (("source" = ANY (ARRAY['google'::"text", 'manual'::"text", 'ai_generated'::"text"]))),
    CONSTRAINT "tourist_destinations_age_check" CHECK (((("age_min" IS NULL) OR ("age_min" >= 0)) AND (("age_max" IS NULL) OR ("age_max" >= "age_min")))),
    CONSTRAINT "tourist_destinations_crowd_level_check" CHECK ((("crowd_level" IS NULL) OR ("crowd_level" = ANY (ARRAY['low'::"text", 'moderate'::"text", 'high'::"text", 'very_high'::"text"])))),
    CONSTRAINT "tourist_destinations_difficulty_level_check" CHECK ((("difficulty_level" IS NULL) OR ("difficulty_level" = ANY (ARRAY['easy'::"text", 'moderate'::"text", 'hard'::"text", 'expert'::"text"])))),
    CONSTRAINT "tourist_destinations_group_size_check" CHECK (((("group_size_min" IS NULL) OR ("group_size_min" >= 1)) AND (("group_size_max" IS NULL) OR ("group_size_max" >= "group_size_min"))))
);


ALTER TABLE "public"."tourist_destinations" OWNER TO "postgres";


COMMENT ON TABLE "public"."tourist_destinations" IS 'tourist destinations table for attractions, landmarks, POIs, and points of interest. includes destination-specific fields like entry fees, visit duration, and categories.';



COMMENT ON COLUMN "public"."tourist_destinations"."subcategory" IS 'specific subcategory within main category';



COMMENT ON COLUMN "public"."tourist_destinations"."category" IS 'main category: museum, park, landmark, nature, culture, etc.';



COMMENT ON COLUMN "public"."tourist_destinations"."entry_fee" IS 'text description of entry fee: Free, Paid, Low, etc.';



COMMENT ON COLUMN "public"."tourist_destinations"."estimated_visit_duration" IS 'estimated time to visit: 30 min, 1-2 hrs, Half day, etc.';



COMMENT ON COLUMN "public"."tourist_destinations"."best_for" IS 'array of use cases: Families, Art, Hiking, Views, etc.';
...
CREATE TABLE IF NOT EXISTS "public"."trip_items" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "trip_items_item_type_check" CHECK (("item_type" = ANY (ARRAY['event'::"text", 'restaurant'::"text", 'rental'::"text", 'poi'::"text", 'other'::"text"]))),
...
COMMENT ON TABLE "public"."trip_items" IS 'polymorphic table linking trips to events, restaurants, rentals, or other items. supports flexible metadata storage.';
...
CREATE TABLE IF NOT EXISTS "public"."trips" (
    ...
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "deleted_at" timestamp with time zone,
    CONSTRAINT "trips_status_check" CHECK (("status" = ANY (ARRAY['planning'::"text", 'active'::"text", 'completed'::"text", 'cancelled'::"text"]))),
...
COMMENT ON TABLE "public"."trips" IS 'trip containers. each trip represents a planned or completed journey with dates, budget, and status tracking.';
...
CREATE TABLE IF NOT EXISTS "public"."user_preferences" (
    ...
    "price_range_preference" "text",
    "ambiance_preferences" "text"[] DEFAULT '{}'::"text"[],
    "event_categories" "text"[] DEFAULT '{}'::"text"[],
    "preferred_event_times" "text"[] DEFAULT '{}'::"text"[],
    "event_price_range" "text",
    "vehicle_types" "text"[] DEFAULT '{}'::"text"[],
    "rental_features" "text"[] DEFAULT '{}'::"text"[],
    "travel_style" "text"[] DEFAULT '{}'::"text"[],
    "adventure_level" "text",
    "ai_proactivity_level" "text" DEFAULT 'medium'::"text",
    "ai_suggestion_frequency" "text" DEFAULT 'daily'::"text",
    "default_budget_per_day" numeric(10,2),
    "default_currency" "text" DEFAULT 'USD'::"text",
    "notification_preferences" "jsonb" DEFAULT '{"sms": false, "push": true, "email": true, "trip_updates": true, "ai_suggestions": true}'::"jsonb",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "user_preferences_adventure_level_check" CHECK (("adventure_level" = ANY (ARRAY['low'::"text", 'medium'::"text", 'high'::"text"]))),
    CONSTRAINT "user_preferences_ai_proactivity_level_check" CHECK (("ai_proactivity_level" = ANY (ARRAY['low'::"text", 'medium'::"text", 'high'::"text"]))),
    CONSTRAINT "user_preferences_ai_suggestion_frequency_check" CHECK (("ai_suggestion_frequency" = ANY (ARRAY['never'::"text", 'daily'::"text", 'weekly'::"text", 'always'::"text"]))),
    CONSTRAINT "user_preferences_event_price_range_check" CHECK (("event_price_range" = ANY (ARRAY['free'::"text", 'low'::"text", 'medium'::"text", 'high'::"text"]))),
    CONSTRAINT "user_preferences_price_range_preference_check" CHECK (("price_range_preference" = ANY (ARRAY['$'::"text", '$$'::"text", '$$$'::"text", '$$$$'::"text"])))
);
...
COMMENT ON TABLE "public"."user_preferences" IS 'user AI preferences and settings. stores dining, event, rental, and travel style preferences for personalization.';
...
COMMENT ON TABLE "public"."whatsapp_conversations" IS 'Track WhatsApp conversations (minimal for MVP)';
...
CREATE TABLE IF NOT EXISTS "public"."whatsapp_messages" (
    ...
    CONSTRAINT "whatsapp_messages_sender_check" CHECK (("sender" = ANY (ARRAY['user'::"text", 'assistant'::"text"]))),
...
COMMENT ON TABLE "public"."whatsapp_messages" IS 'Store incoming and outgoing WhatsApp messages for conversation history and analytics';
...
ALTER TABLE ONLY "public"."car_rentals"
    ADD CONSTRAINT "car_rentals_slug_key" UNIQUE ("slug");
...
ALTER TABLE ONLY "public"."events"
    ADD CONSTRAINT "events_google_place_id_key" UNIQUE ("google_place_id");
...
ALTER TABLE ONLY "public"."rental_freshness_log"
    ADD CONSTRAINT "rental_freshness_log_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."rental_listing_images"
    ADD CONSTRAINT "rental_listing_images_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."rental_listing_sources"
    ADD CONSTRAINT "rental_listing_sources_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."rental_listing_sources"
    ADD CONSTRAINT "rental_listing_sources_source_key_key" UNIQUE ("source_key");



ALTER TABLE ONLY "public"."rental_search_sessions"
    ADD CONSTRAINT "rental_search_sessions_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."budget_tracking"
    ADD CONSTRAINT "unique_budget_per_trip" UNIQUE ("trip_id");



ALTER TABLE ONLY "public"."ai_context"
    ADD CONSTRAINT "unique_context_per_conversation" UNIQUE ("conversation_id");



ALTER TABLE ONLY "public"."trip_items"
    ADD CONSTRAINT "unique_trip_item" UNIQUE ("trip_id", "item_type", "source_id");



ALTER TABLE ONLY "public"."saved_places"
    ADD CONSTRAINT "unique_user_location" UNIQUE ("user_id", "location_type", "location_id");
...
CREATE INDEX "idx_agent_jobs_expires_at" ON "public"."agent_jobs" USING "btree" ("expires_at") WHERE ("status" = ANY (ARRAY['queued'::"text", 'running'::"text"]));



CREATE INDEX "idx_agent_jobs_status_created" ON "public"."agent_jobs" USING "btree" ("status", "run_after", "created_at") WHERE ("status" = 'queued'::"text");



CREATE INDEX "idx_agent_jobs_user_created" ON "public"."agent_jobs" USING "btree" ("user_id", "created_at" DESC);



CREATE INDEX "idx_agent_jobs_user_status" ON "public"."agent_jobs" USING "btree" ("user_id", "status");



CREATE INDEX "idx_ai_context_conversation" ON "public"."ai_context" USING "btree" ("conversation_id");



CREATE INDEX "idx_ai_context_current_intent" ON "public"."ai_context" USING "btree" ("current_intent") WHERE ("current_intent" IS NOT NULL);



CREATE INDEX "idx_ai_runs_agent_name" ON "public"."ai_runs" USING "btree" ("agent_name");



CREATE INDEX "idx_ai_runs_agent_type" ON "public"."ai_runs" USING "btree" ("agent_type");



CREATE INDEX "idx_ai_runs_conversation_id" ON "public"."ai_runs" USING "btree" ("conversation_id") WHERE ("conversation_id" IS NOT NULL);
...
CREATE INDEX "idx_ai_runs_user_status" ON "public"."ai_runs" USING "btree" ("user_id", "status", "created_at" DESC);



CREATE INDEX "idx_apartments_available_from" ON "public"."apartments" USING "btree" ("available_from");



CREATE INDEX "idx_apartments_bedrooms" ON "public"."apartments" USING "btree" ("bedrooms");



CREATE INDEX "idx_apartments_freshness_status" ON "public"."apartments" USING "btree" ("freshness_status") WHERE ("freshness_status" IS NOT NULL);



CREATE INDEX "idx_apartments_furnished" ON "public"."apartments" USING "btree" ("furnished");



CREATE INDEX "idx_apartments_last_checked_at" ON "public"."apartments" USING "btree" ("last_checked_at" DESC NULLS LAST);



CREATE INDEX "idx_apartments_location" ON "public"."apartments" USING "gist" ("location");



CREATE INDEX "idx_apartments_neighborhood" ON "public"."apartments" USING "btree" ("neighborhood");



CREATE INDEX "idx_apartments_price_monthly" ON "public"."apartments" USING "btree" ("price_monthly");



CREATE INDEX "idx_apartments_rental_search" ON "public"."apartments" USING "btree" ("neighborhood", "bedrooms", "price_monthly") WHERE ("status" = 'active'::"text");



COMMENT ON INDEX "public"."idx_apartments_rental_search" IS 'composite index for common rental search filters';
...
CREATE INDEX "idx_bookings_confirmation" ON "public"."bookings" USING "btree" ("confirmation_code") WHERE ("confirmation_code" IS NOT NULL);
...
CREATE INDEX "idx_bookings_trip" ON "public"."bookings" USING "btree" ("trip_id") WHERE ("trip_id" IS NOT NULL);
...
CREATE INDEX "idx_budget_tracking_alerts" ON "public"."budget_tracking" USING "btree" ((("total_spent" / NULLIF("total_budget", (0)::numeric)))) WHERE (("total_spent" / NULLIF("total_budget", (0)::numeric)) >= 0.80);



CREATE INDEX "idx_budget_tracking_trip_id" ON "public"."budget_tracking" USING "btree" ("trip_id");



CREATE INDEX "idx_car_rentals_available_from" ON "public"."car_rentals" USING "btree" ("available_from");



CREATE INDEX "idx_car_rentals_make" ON "public"."car_rentals" USING "btree" ("make");



CREATE INDEX "idx_car_rentals_price_daily" ON "public"."car_rentals" USING "btree" ("price_daily");



CREATE INDEX "idx_car_rentals_status" ON "public"."car_rentals" USING "btree" ("status");



CREATE INDEX "idx_car_rentals_transmission" ON "public"."car_rentals" USING "btree" ("transmission");



CREATE INDEX "idx_car_rentals_vehicle_type" ON "public"."car_rentals" USING "btree" ("vehicle_type");



CREATE INDEX "idx_collections_public" ON "public"."collections" USING "btree" ("is_public") WHERE (("is_public" = true) AND ("deleted_at" IS NULL));
...
CREATE INDEX "idx_collections_user_id" ON "public"."collections" USING "btree" ("user_id") WHERE ("deleted_at" IS NULL);



CREATE INDEX "idx_conflicts_severity" ON "public"."conflict_resolutions" USING "btree" ("severity" DESC);



CREATE INDEX "idx_conflicts_status" ON "public"."conflict_resolutions" USING "btree" ("status");



CREATE INDEX "idx_conflicts_trip_id" ON "public"."conflict_resolutions" USING "btree" ("trip_id");



CREATE INDEX "idx_conflicts_type" ON "public"."conflict_resolutions" USING "btree" ("type");



CREATE INDEX "idx_conflicts_unresolved" ON "public"."conflict_resolutions" USING "btree" ("trip_id", "status") WHERE ("status" = ANY (ARRAY['detected'::"public"."resolution_status", 'pending_review'::"public"."resolution_status"]));



CREATE INDEX "idx_conflicts_user_id" ON "public"."conflict_resolutions" USING "btree" ("user_id");



CREATE INDEX "idx_conversations_agent_type" ON "public"."conversations" USING "btree" ("agent_type");
...
CREATE INDEX "idx_conversations_trip_id" ON "public"."conversations" USING "btree" ("trip_id") WHERE ("deleted_at" IS NULL);



CREATE INDEX "idx_conversations_user_id" ON "public"."conversations" USING "btree" ("user_id") WHERE ("deleted_at" IS NULL);



CREATE INDEX "idx_conversations_user_last_message" ON "public"."conversations" USING "btree" ("user_id", "last_message_at" DESC) WHERE ("status" = 'active'::"public"."conversation_status");
...
CREATE INDEX "idx_events_city" ON "public"."events" USING "btree" ("city") WHERE ("city" IS NOT NULL);
...
CREATE INDEX "idx_events_google_place_id" ON "public"."events" USING "btree" ("google_place_id") WHERE ("google_place_id" IS NOT NULL);



CREATE INDEX "idx_events_lat_lng" ON "public"."events" USING "btree" ("latitude", "longitude") WHERE (("latitude" IS NOT NULL) AND ("longitude" IS NOT NULL));



CREATE INDEX "idx_events_rating" ON "public"."events" USING "btree" ("rating" DESC NULLS LAST) WHERE ("rating" IS NOT NULL);
...
CREATE INDEX "idx_events_time_range" ON "public"."events" USING "btree" ("event_start_time", "event_end_time") WHERE ("event_end_time" IS NOT NULL);



CREATE INDEX "idx_events_type" ON "public"."events" USING "btree" ("event_type") WHERE ("event_type" IS NOT NULL);
...
CREATE INDEX "idx_proactive_suggestions_created_at" ON "public"."proactive_suggestions" USING "btree" ("created_at" DESC);



CREATE INDEX "idx_proactive_suggestions_status" ON "public"."proactive_suggestions" USING "btree" ("status");



CREATE INDEX "idx_proactive_suggestions_trip_id" ON "public"."proactive_suggestions" USING "btree" ("trip_id");



CREATE INDEX "idx_proactive_suggestions_user_id" ON "public"."proactive_suggestions" USING "btree" ("user_id");
...
CREATE INDEX "idx_profiles_last_active" ON "public"."profiles" USING "btree" ("last_active_at" DESC) WHERE ("deleted_at" IS NULL);



CREATE INDEX "idx_profiles_role" ON "public"."profiles" USING "btree" ("role") WHERE ("role" <> 'user'::"public"."user_role");



CREATE INDEX "idx_rental_freshness_log_checked_at" ON "public"."rental_freshness_log" USING "btree" ("checked_at" DESC);



CREATE INDEX "idx_rental_freshness_log_listing_id" ON "public"."rental_freshness_log" USING "btree" ("listing_id");



CREATE INDEX "idx_rental_freshness_log_status" ON "public"."rental_freshness_log" USING "btree" ("listing_id", "status") WHERE ("status" <> 'active'::"text");



CREATE INDEX "idx_rental_listing_images_listing_id" ON "public"."rental_listing_images" USING "btree" ("listing_id");



CREATE INDEX "idx_rental_listing_images_sort" ON "public"."rental_listing_images" USING "btree" ("listing_id", "sort_order");



CREATE INDEX "idx_rental_listing_sources_enabled" ON "public"."rental_listing_sources" USING "btree" ("is_enabled") WHERE ("is_enabled" = true);



CREATE INDEX "idx_rental_listing_sources_key" ON "public"."rental_listing_sources" USING "btree" ("source_key");



CREATE INDEX "idx_rental_search_sessions_created_at" ON "public"."rental_search_sessions" USING "btree" ("created_at" DESC);



CREATE INDEX "idx_rental_search_sessions_user_id" ON "public"."rental_search_sessions" USING "btree" ("user_id");



CREATE INDEX "idx_rentals_active" ON "public"."rentals" USING "btree" ("is_active") WHERE ("is_active" = true);



CREATE INDEX "idx_rentals_availability" ON "public"."rentals" USING "btree" ("availability_start", "availability_end") WHERE ("availability_start" IS NOT NULL);



CREATE INDEX "idx_rentals_city" ON "public"."rentals" USING "btree" ("city") WHERE ("city" IS NOT NULL);



CREATE INDEX "idx_rentals_created_by" ON "public"."rentals" USING "btree" ("created_by") WHERE ("created_by" IS NOT NULL);



CREATE INDEX "idx_rentals_daily_rate" ON "public"."rentals" USING "btree" ("daily_rate") WHERE ("daily_rate" IS NOT NULL);



CREATE INDEX "idx_rentals_lat_lng" ON "public"."rentals" USING "btree" ("latitude", "longitude") WHERE (("latitude" IS NOT NULL) AND ("longitude" IS NOT NULL));



CREATE INDEX "idx_rentals_rating" ON "public"."rentals" USING "btree" ("rating" DESC NULLS LAST) WHERE ("rating" IS NOT NULL);



CREATE INDEX "idx_rentals_tags" ON "public"."rentals" USING "gin" ("tags");



CREATE INDEX "idx_rentals_vehicle_type" ON "public"."rentals" USING "btree" ("vehicle_type") WHERE ("vehicle_type" IS NOT NULL);
...
CREATE INDEX "idx_restaurants_created_by" ON "public"."restaurants" USING "btree" ("created_by") WHERE ("created_by" IS NOT NULL);
...
CREATE INDEX "idx_restaurants_lat_lng" ON "public"."restaurants" USING "btree" ("latitude", "longitude") WHERE (("latitude" IS NOT NULL) AND ("longitude" IS NOT NULL));



CREATE INDEX "idx_restaurants_price_level" ON "public"."restaurants" USING "btree" ("price_level");



CREATE INDEX "idx_restaurants_rating" ON "public"."restaurants" USING "btree" ("rating" DESC NULLS LAST) WHERE ("rating" IS NOT NULL);
...
CREATE INDEX "idx_saved_places_collection" ON "public"."saved_places" USING "btree" ("collection_id") WHERE ("collection_id" IS NOT NULL);



CREATE INDEX "idx_saved_places_favorites" ON "public"."saved_places" USING "btree" ("user_id", "is_favorite") WHERE ("is_favorite" = true);



CREATE INDEX "idx_saved_places_location" ON "public"."saved_places" USING "btree" ("location_type", "location_id");



CREATE INDEX "idx_saved_places_tags" ON "public"."saved_places" USING "gin" ("tags");



CREATE INDEX "idx_saved_places_trip_id" ON "public"."saved_places" USING "btree" ("trip_id");
...
CREATE INDEX "idx_saved_places_user_location" ON "public"."saved_places" USING "btree" ("user_id", "location_type");



CREATE INDEX "idx_saved_places_user_trip" ON "public"."saved_places" USING "btree" ("user_id", "trip_id");



CREATE INDEX "idx_tourist_destinations_active" ON "public"."tourist_destinations" USING "btree" ("is_active") WHERE ("is_active" = true);



CREATE INDEX "idx_tourist_destinations_booking_required" ON "public"."tourist_destinations" USING "btree" ("booking_required") WHERE ("booking_required" = true);



CREATE INDEX "idx_tourist_destinations_category" ON "public"."tourist_destinations" USING "btree" ("category") WHERE ("category" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_city" ON "public"."tourist_destinations" USING "btree" ("city") WHERE ("city" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_created_by" ON "public"."tourist_destinations" USING "btree" ("created_by") WHERE ("created_by" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_family_friendly" ON "public"."tourist_destinations" USING "btree" ("family_friendly") WHERE ("family_friendly" = true);



CREATE INDEX "idx_tourist_destinations_google_place_id" ON "public"."tourist_destinations" USING "btree" ("google_place_id") WHERE ("google_place_id" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_lat_lng" ON "public"."tourist_destinations" USING "btree" ("latitude", "longitude") WHERE (("latitude" IS NOT NULL) AND ("longitude" IS NOT NULL));



CREATE INDEX "idx_tourist_destinations_rating" ON "public"."tourist_destinations" USING "btree" ("rating" DESC NULLS LAST) WHERE ("rating" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_seasonal" ON "public"."tourist_destinations" USING "btree" ("seasonal_start", "seasonal_end") WHERE ("seasonal_start" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_subcategory" ON "public"."tourist_destinations" USING "btree" ("subcategory") WHERE ("subcategory" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_tags" ON "public"."tourist_destinations" USING "gin" ("tags");



CREATE INDEX "idx_tourist_destinations_tour_operator" ON "public"."tourist_destinations" USING "btree" ("tour_operator") WHERE ("tour_operator" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_tripadvisor_id" ON "public"."tourist_destinations" USING "btree" ("tripadvisor_id") WHERE ("tripadvisor_id" IS NOT NULL);



CREATE INDEX "idx_tourist_destinations_wheelchair_accessible" ON "public"."tourist_destinations" USING "btree" ("wheelchair_accessible") WHERE ("wheelchair_accessible" = true);



CREATE INDEX "idx_trip_items_created_by" ON "public"."trip_items" USING "btree" ("created_by") WHERE ("created_by" IS NOT NULL);



CREATE INDEX "idx_trip_items_dates" ON "public"."trip_items" USING "btree" ("start_at", "end_at");



CREATE INDEX "idx_trip_items_trip" ON "public"."trip_items" USING "btree" ("trip_id");



CREATE INDEX "idx_trip_items_type" ON "public"."trip_items" USING "btree" ("item_type", "source_id");



CREATE INDEX "idx_trips_destination" ON "public"."trips" USING "btree" ("destination") WHERE ("destination" IS NOT NULL);
...
CREATE INDEX "idx_trips_user_id" ON "public"."trips" USING "btree" ("user_id") WHERE ("deleted_at" IS NULL);
...
CREATE INDEX "idx_user_preferences_dietary" ON "public"."user_preferences" USING "gin" ("dietary_restrictions");



CREATE INDEX "idx_user_preferences_events" ON "public"."user_preferences" USING "gin" ("event_categories");
...
COMMENT ON INDEX "public"."idx_user_preferences_user_id" IS 'Primary user lookup index for user_preferences';
...
CREATE INDEX "idx_whatsapp_conversations_last_message_at" ON "public"."whatsapp_conversations" USING "btree" ("last_message_at" DESC);



CREATE INDEX "idx_whatsapp_conversations_phone_number" ON "public"."whatsapp_conversations" USING "btree" ("phone_number");
...
CREATE INDEX "idx_whatsapp_messages_external_id" ON "public"."whatsapp_messages" USING "btree" ("external_id") WHERE ("external_id" IS NOT NULL);



CREATE INDEX "idx_whatsapp_messages_phone_created" ON "public"."whatsapp_messages" USING "btree" ("phone_number", "created_at" DESC);
...
CREATE UNIQUE INDEX "uq_whatsapp_messages_external_id" ON "public"."whatsapp_messages" USING "btree" ("external_id") WHERE ("external_id" IS NOT NULL);



CREATE OR REPLACE TRIGGER "agent_jobs_realtime_broadcast" AFTER INSERT OR UPDATE ON "public"."agent_jobs" FOR EACH ROW EXECUTE FUNCTION "public"."realtime_broadcast_agent_jobs"();



CREATE OR REPLACE TRIGGER "ai_context_updated_at" BEFORE UPDATE ON "public"."ai_context" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();



CREATE OR REPLACE TRIGGER "apartments_updated_at" BEFORE UPDATE ON "public"."apartments" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();
...
CREATE OR REPLACE TRIGGER "budget_tracking_updated_at" BEFORE UPDATE ON "public"."budget_tracking" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();



CREATE OR REPLACE TRIGGER "car_rentals_updated_at" BEFORE UPDATE ON "public"."car_rentals" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();



CREATE OR REPLACE TRIGGER "collections_updated_at" BEFORE UPDATE ON "public"."collections" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();



CREATE OR REPLACE TRIGGER "conflict_resolutions_updated_at" BEFORE UPDATE ON "public"."conflict_resolutions" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();
...
CREATE OR REPLACE TRIGGER "messages_realtime_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."messages" FOR EACH ROW EXECUTE FUNCTION "public"."realtime_broadcast_messages"();
...
CREATE OR REPLACE TRIGGER "rentals_updated_at" BEFORE UPDATE ON "public"."rentals" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();
...
CREATE OR REPLACE TRIGGER "tourist_destinations_updated_at" BEFORE UPDATE ON "public"."tourist_destinations" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();



CREATE OR REPLACE TRIGGER "trigger_agent_jobs_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."agent_jobs" FOR EACH ROW EXECUTE FUNCTION "public"."broadcast_agent_jobs_changes"();



CREATE OR REPLACE TRIGGER "trigger_broadcast_proactive_suggestions" AFTER INSERT OR DELETE OR UPDATE ON "public"."proactive_suggestions" FOR EACH ROW EXECUTE FUNCTION "public"."broadcast_proactive_suggestions_changes"();



CREATE OR REPLACE TRIGGER "trigger_messages_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."messages" FOR EACH ROW EXECUTE FUNCTION "public"."broadcast_messages_changes"();



CREATE OR REPLACE TRIGGER "trigger_suggestions_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."proactive_suggestions" FOR EACH ROW EXECUTE FUNCTION "public"."broadcast_suggestions_changes"();



CREATE OR REPLACE TRIGGER "trigger_trip_items_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."trip_items" FOR EACH ROW EXECUTE FUNCTION "public"."broadcast_trip_items_changes"();



CREATE OR REPLACE TRIGGER "trigger_trips_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."trips" FOR EACH ROW EXECUTE FUNCTION "public"."broadcast_trips_changes"();



CREATE OR REPLACE TRIGGER "trip_items_realtime_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."trip_items" FOR EACH ROW EXECUTE FUNCTION "public"."realtime_broadcast_trip_items"();



CREATE OR REPLACE TRIGGER "trip_items_updated_at" BEFORE UPDATE ON "public"."trip_items" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();



CREATE OR REPLACE TRIGGER "trips_realtime_broadcast" AFTER INSERT OR DELETE OR UPDATE ON "public"."trips" FOR EACH ROW EXECUTE FUNCTION "public"."realtime_broadcast_trips"();
...
CREATE OR REPLACE TRIGGER "update_conversation_on_message" AFTER INSERT ON "public"."messages" FOR EACH ROW EXECUTE FUNCTION "public"."update_conversation_stats"();
...
CREATE OR REPLACE TRIGGER "update_whatsapp_conversations_updated_at" BEFORE UPDATE ON "public"."whatsapp_conversations" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at"();
...
ALTER TABLE ONLY "public"."agent_jobs"
    ADD CONSTRAINT "agent_jobs_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."ai_context"
    ADD CONSTRAINT "ai_context_conversation_id_fkey" FOREIGN KEY ("conversation_id") REFERENCES "public"."conversations"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."ai_runs"
    ADD CONSTRAINT "ai_runs_conversation_id_fkey" FOREIGN KEY ("conversation_id") REFERENCES "public"."conversations"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."ai_runs"
    ADD CONSTRAINT "ai_runs_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."apartments"
    ADD CONSTRAINT "apartments_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");



ALTER TABLE ONLY "public"."apartments"
    ADD CONSTRAINT "apartments_host_id_fkey" FOREIGN KEY ("host_id") REFERENCES "auth"."users"("id");
...
ALTER TABLE ONLY "public"."budget_tracking"
    ADD CONSTRAINT "budget_tracking_trip_id_fkey" FOREIGN KEY ("trip_id") REFERENCES "public"."trips"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."car_rentals"
    ADD CONSTRAINT "car_rentals_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id");
...
ALTER TABLE ONLY "public"."conflict_resolutions"
    ADD CONSTRAINT "conflict_resolutions_trip_id_fkey" FOREIGN KEY ("trip_id") REFERENCES "public"."trips"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."conflict_resolutions"
    ADD CONSTRAINT "conflict_resolutions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."conversations"
    ADD CONSTRAINT "conversations_trip_id_fkey" FOREIGN KEY ("trip_id") REFERENCES "public"."trips"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."proactive_suggestions"
    ADD CONSTRAINT "proactive_suggestions_trip_id_fkey" FOREIGN KEY ("trip_id") REFERENCES "public"."trips"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."proactive_suggestions"
    ADD CONSTRAINT "proactive_suggestions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."rental_freshness_log"
    ADD CONSTRAINT "rental_freshness_log_listing_id_fkey" FOREIGN KEY ("listing_id") REFERENCES "public"."apartments"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."rental_listing_images"
    ADD CONSTRAINT "rental_listing_images_listing_id_fkey" FOREIGN KEY ("listing_id") REFERENCES "public"."apartments"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."rental_search_sessions"
    ADD CONSTRAINT "rental_search_sessions_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "auth"."users"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."rentals"
    ADD CONSTRAINT "rentals_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."saved_places"
    ADD CONSTRAINT "saved_places_collection_id_fkey" FOREIGN KEY ("collection_id") REFERENCES "public"."collections"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."saved_places"
    ADD CONSTRAINT "saved_places_trip_id_fkey" FOREIGN KEY ("trip_id") REFERENCES "public"."trips"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."tourist_destinations"
    ADD CONSTRAINT "tourist_destinations_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."trip_items"
    ADD CONSTRAINT "trip_items_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "auth"."users"("id") ON DELETE SET NULL;
...
CREATE POLICY "Service role can manage all suggestions" ON "public"."proactive_suggestions" USING (("auth"."role"() = 'service_role'::"text"));
...
CREATE POLICY "Users can cancel own jobs" ON "public"."agent_jobs" FOR UPDATE TO "authenticated" USING (((( SELECT "auth"."uid"() AS "uid") = "user_id") AND ("status" = 'queued'::"text"))) WITH CHECK (((( SELECT "auth"."uid"() AS "uid") = "user_id") AND ("status" = 'cancelled'::"text")));



COMMENT ON POLICY "Users can cancel own jobs" ON "public"."agent_jobs" IS 'RLS policy using performance pattern (select auth.uid()) for optimal query performance';



CREATE POLICY "Users can create own jobs" ON "public"."agent_jobs" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



COMMENT ON POLICY "Users can create own jobs" ON "public"."agent_jobs" IS 'RLS policy using performance pattern (select auth.uid()) for optimal query performance';



CREATE POLICY "Users can update their own suggestions" ON "public"."proactive_suggestions" FOR UPDATE USING ((( SELECT "auth"."uid"() AS "uid") = "user_id")) WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "Users can view own jobs" ON "public"."agent_jobs" FOR SELECT TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



COMMENT ON POLICY "Users can view own jobs" ON "public"."agent_jobs" IS 'RLS policy using performance pattern (select auth.uid()) for optimal query performance';



CREATE POLICY "Users can view their own suggestions" ON "public"."proactive_suggestions" FOR SELECT USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "admins_can_delete_events" ON "public"."events" FOR DELETE TO "authenticated" USING (( SELECT "public"."is_admin"() AS "is_admin"));



CREATE POLICY "admins_can_delete_rentals" ON "public"."rentals" FOR DELETE TO "authenticated" USING (( SELECT "public"."is_admin"() AS "is_admin"));



CREATE POLICY "admins_can_delete_restaurants" ON "public"."restaurants" FOR DELETE TO "authenticated" USING (( SELECT "public"."is_admin"() AS "is_admin"));



CREATE POLICY "admins_can_delete_tourist_destinations" ON "public"."tourist_destinations" FOR DELETE TO "authenticated" USING (( SELECT "public"."is_admin"() AS "is_admin"));



CREATE POLICY "admins_view_all_roles" ON "public"."user_roles" FOR SELECT TO "authenticated" USING ("public"."is_admin"());
...
CREATE POLICY "anon_can_view_active_events" ON "public"."events" FOR SELECT TO "anon" USING (("is_active" = true));



CREATE POLICY "anon_can_view_active_rentals" ON "public"."rentals" FOR SELECT TO "anon" USING (("is_active" = true));



CREATE POLICY "anon_can_view_active_restaurants" ON "public"."restaurants" FOR SELECT TO "anon" USING (("is_active" = true));



CREATE POLICY "anon_can_view_public_collections" ON "public"."collections" FOR SELECT TO "anon" USING ((("is_public" = true) AND ("deleted_at" IS NULL)));
...
CREATE POLICY "anyone_can_view_active_apartments" ON "public"."apartments" FOR SELECT USING ((("status" = 'active'::"text") OR ("status" = 'featured'::"text")));



CREATE POLICY "anyone_can_view_active_car_rentals" ON "public"."car_rentals" FOR SELECT USING ((("status" = 'active'::"text") OR ("status" = 'featured'::"text")));
...
CREATE POLICY "authenticated_can_view_active_events" ON "public"."events" FOR SELECT TO "authenticated" USING (("is_active" = true));



CREATE POLICY "authenticated_can_view_active_rentals" ON "public"."rentals" FOR SELECT TO "authenticated" USING (("is_active" = true));



CREATE POLICY "authenticated_can_view_active_restaurants" ON "public"."restaurants" FOR SELECT TO "authenticated" USING (("is_active" = true));



CREATE POLICY "authenticated_can_view_all_apartments" ON "public"."apartments" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "authenticated_can_view_all_car_rentals" ON "public"."car_rentals" FOR SELECT TO "authenticated" USING (true);



CREATE POLICY "authenticated_delete_own_rental_search_sessions" ON "public"."rental_search_sessions" FOR DELETE TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "authenticated_insert_own_rental_search_sessions" ON "public"."rental_search_sessions" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "authenticated_select_own_rental_search_sessions" ON "public"."rental_search_sessions" FOR SELECT TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));
...
CREATE POLICY "authenticated_update_own_rental_search_sessions" ON "public"."rental_search_sessions" FOR UPDATE TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id")) WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "authenticated_users_can_create_bookings" ON "public"."bookings" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "authenticated_users_can_delete_budget_for_own_trips" ON "public"."budget_tracking" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "budget_tracking"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));



CREATE POLICY "authenticated_users_can_delete_conflicts_in_own_trips" ON "public"."conflict_resolutions" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_context_for_own_conversations" ON "public"."ai_context" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "ai_context"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));



CREATE POLICY "authenticated_users_can_delete_messages_in_own_conversations" ON "public"."messages" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "messages"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));



CREATE POLICY "authenticated_users_can_delete_own_ai_runs" ON "public"."ai_runs" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_own_collections" ON "public"."collections" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_own_conversations" ON "public"."conversations" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_own_preferences" ON "public"."user_preferences" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_own_saved_places" ON "public"."saved_places" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_own_trips" ON "public"."trips" FOR DELETE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_delete_trip_items" ON "public"."trip_items" FOR DELETE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "trip_items"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));



CREATE POLICY "authenticated_users_can_insert_budget_for_own_trips" ON "public"."budget_tracking" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "budget_tracking"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_insert_conflicts_in_own_trips" ON "public"."conflict_resolutions" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_context_for_own_conversations" ON "public"."ai_context" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "ai_context"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_insert_messages_in_own_conversations" ON "public"."messages" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "messages"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_insert_own_ai_runs" ON "public"."ai_runs" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_own_collections" ON "public"."collections" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_own_conversations" ON "public"."conversations" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_own_preferences" ON "public"."user_preferences" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_own_profile" ON "public"."profiles" FOR INSERT TO "authenticated" WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "id"));



CREATE POLICY "authenticated_users_can_insert_own_saved_places" ON "public"."saved_places" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_own_trips" ON "public"."trips" FOR INSERT TO "authenticated" WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_insert_trip_items" ON "public"."trip_items" FOR INSERT TO "authenticated" WITH CHECK ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "trip_items"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_update_budget_for_own_trips" ON "public"."budget_tracking" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "budget_tracking"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
...
CREATE POLICY "authenticated_users_can_update_conflicts_in_own_trips" ON "public"."conflict_resolutions" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_update_context_for_own_conversations" ON "public"."ai_context" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "ai_context"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))))))) WITH CHECK ((EXISTS ( SELECT 1
...
CREATE POLICY "authenticated_users_can_update_messages_in_own_conversations" ON "public"."messages" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "messages"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))))))) WITH CHECK ((EXISTS ( SELECT 1
...
CREATE POLICY "authenticated_users_can_update_own_ai_runs" ON "public"."ai_runs" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "authenticated_users_can_update_own_bookings" ON "public"."bookings" FOR UPDATE TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id")) WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "authenticated_users_can_update_own_collections" ON "public"."collections" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "authenticated_users_can_update_own_conversations" ON "public"."conversations" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "authenticated_users_can_update_own_preferences" ON "public"."user_preferences" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "authenticated_users_can_update_own_profile" ON "public"."profiles" FOR UPDATE TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "id")) WITH CHECK ((( SELECT "auth"."uid"() AS "uid") = "id"));



CREATE POLICY "authenticated_users_can_update_own_saved_places" ON "public"."saved_places" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "authenticated_users_can_update_own_trips" ON "public"."trips" FOR UPDATE TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))) WITH CHECK (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));
...
CREATE POLICY "authenticated_users_can_update_trip_items" ON "public"."trip_items" FOR UPDATE TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "trip_items"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))))))) WITH CHECK ((EXISTS ( SELECT 1
...
CREATE POLICY "authenticated_users_can_view_budget_for_own_trips" ON "public"."budget_tracking" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "budget_tracking"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_view_conflicts_in_own_trips" ON "public"."conflict_resolutions" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_view_context_for_own_conversations" ON "public"."ai_context" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "ai_context"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_view_messages_in_own_conversations" ON "public"."messages" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."conversations"
  WHERE (("conversations"."id" = "messages"."conversation_id") AND ("conversations"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "authenticated_users_can_view_own_ai_runs" ON "public"."ai_runs" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_view_own_bookings" ON "public"."bookings" FOR SELECT TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "user_id"));



CREATE POLICY "authenticated_users_can_view_own_conversations" ON "public"."conversations" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_view_own_or_public_collections" ON "public"."collections" FOR SELECT TO "authenticated" USING ((("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))) OR ("is_public" = true)));



CREATE POLICY "authenticated_users_can_view_own_preferences" ON "public"."user_preferences" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_view_own_profile" ON "public"."profiles" FOR SELECT TO "authenticated" USING ((( SELECT "auth"."uid"() AS "uid") = "id"));



CREATE POLICY "authenticated_users_can_view_own_saved_places" ON "public"."saved_places" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_view_own_trips" ON "public"."trips" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "profiles"."id"
   FROM "public"."profiles"
  WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid")))));



CREATE POLICY "authenticated_users_can_view_trip_items" ON "public"."trip_items" FOR SELECT TO "authenticated" USING ((EXISTS ( SELECT 1
   FROM "public"."trips"
  WHERE (("trips"."id" = "trip_items"."trip_id") AND ("trips"."user_id" = ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."id" = ( SELECT "auth"."uid"() AS "uid"))))))));
...
CREATE POLICY "moderators_can_insert_events" ON "public"."events" FOR INSERT TO "authenticated" WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_insert_rentals" ON "public"."rentals" FOR INSERT TO "authenticated" WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_insert_restaurants" ON "public"."restaurants" FOR INSERT TO "authenticated" WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_insert_tourist_destinations" ON "public"."tourist_destinations" FOR INSERT TO "authenticated" WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_update_events" ON "public"."events" FOR UPDATE TO "authenticated" USING (( SELECT "public"."is_moderator"() AS "is_moderator")) WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_update_rentals" ON "public"."rentals" FOR UPDATE TO "authenticated" USING (( SELECT "public"."is_moderator"() AS "is_moderator")) WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_update_restaurants" ON "public"."restaurants" FOR UPDATE TO "authenticated" USING (( SELECT "public"."is_moderator"() AS "is_moderator")) WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));



CREATE POLICY "moderators_can_update_tourist_destinations" ON "public"."tourist_destinations" FOR UPDATE TO "authenticated" USING (( SELECT "public"."is_moderator"() AS "is_moderator")) WITH CHECK (( SELECT "public"."is_moderator"() AS "is_moderator"));
...
CREATE POLICY "service_role_full_access_apartments" ON "public"."apartments" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role_full_access_car_rentals" ON "public"."car_rentals" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "service_role_manage_events" ON "public"."events" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role_manage_rentals" ON "public"."rentals" TO "service_role" USING (true) WITH CHECK (true);



CREATE POLICY "service_role_manage_restaurants" ON "public"."restaurants" TO "service_role" USING (true) WITH CHECK (true);
...
CREATE POLICY "super_admins_delete_roles" ON "public"."user_roles" FOR DELETE TO "authenticated" USING ("public"."has_role"(( SELECT "auth"."uid"() AS "uid"), 'super_admin'::"public"."user_role"));



CREATE POLICY "super_admins_insert_roles" ON "public"."user_roles" FOR INSERT TO "authenticated" WITH CHECK ("public"."has_role"(( SELECT "auth"."uid"() AS "uid"), 'super_admin'::"public"."user_role"));



CREATE POLICY "super_admins_update_roles" ON "public"."user_roles" FOR UPDATE TO "authenticated" USING ("public"."has_role"(( SELECT "auth"."uid"() AS "uid"), 'super_admin'::"public"."user_role")) WITH CHECK ("public"."has_role"(( SELECT "auth"."uid"() AS "uid"), 'super_admin'::"public"."user_role"));
...
CREATE POLICY "tourist_destinations_select_anon" ON "public"."tourist_destinations" FOR SELECT TO "anon" USING (("is_active" = true));



CREATE POLICY "tourist_destinations_select_authenticated" ON "public"."tourist_destinations" FOR SELECT TO "authenticated" USING (true);
...
CREATE POLICY "users_view_own_roles" ON "public"."user_roles" FOR SELECT TO "authenticated" USING (("user_id" = ( SELECT "auth"."uid"() AS "uid")));
...
GRANT ALL ON FUNCTION "public"."broadcast_agent_jobs_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."broadcast_agent_jobs_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."broadcast_agent_jobs_changes"() TO "service_role";



GRANT ALL ON FUNCTION "public"."broadcast_messages_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."broadcast_messages_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."broadcast_messages_changes"() TO "service_role";



GRANT ALL ON FUNCTION "public"."broadcast_proactive_suggestions_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."broadcast_proactive_suggestions_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."broadcast_proactive_suggestions_changes"() TO "service_role";



GRANT ALL ON FUNCTION "public"."broadcast_suggestions_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."broadcast_suggestions_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."broadcast_suggestions_changes"() TO "service_role";



GRANT ALL ON FUNCTION "public"."broadcast_trip_items_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."broadcast_trip_items_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."broadcast_trip_items_changes"() TO "service_role";



GRANT ALL ON FUNCTION "public"."broadcast_trips_changes"() TO "anon";
GRANT ALL ON FUNCTION "public"."broadcast_trips_changes"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."broadcast_trips_changes"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."claim_agent_job"("p_worker_id" "text", "p_job_types" "text"[]) TO "anon";
GRANT ALL ON FUNCTION "public"."claim_agent_job"("p_worker_id" "text", "p_job_types" "text"[]) TO "authenticated";
GRANT ALL ON FUNCTION "public"."claim_agent_job"("p_worker_id" "text", "p_job_types" "text"[]) TO "service_role";



GRANT ALL ON FUNCTION "public"."cleanup_expired_agent_jobs"() TO "anon";
GRANT ALL ON FUNCTION "public"."cleanup_expired_agent_jobs"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."cleanup_expired_agent_jobs"() TO "service_role";



GRANT ALL ON FUNCTION "public"."complete_agent_job"("p_job_id" "uuid", "p_result" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."complete_agent_job"("p_job_id" "uuid", "p_result" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."complete_agent_job"("p_job_id" "uuid", "p_result" "jsonb") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."fail_agent_job"("p_job_id" "uuid", "p_error" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."fail_agent_job"("p_job_id" "uuid", "p_error" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."fail_agent_job"("p_job_id" "uuid", "p_error" "text") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."realtime_broadcast_agent_jobs"() TO "anon";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_agent_jobs"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_agent_jobs"() TO "service_role";



GRANT ALL ON FUNCTION "public"."realtime_broadcast_messages"() TO "anon";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_messages"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_messages"() TO "service_role";



GRANT ALL ON FUNCTION "public"."realtime_broadcast_trip_items"() TO "anon";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_trip_items"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_trip_items"() TO "service_role";



GRANT ALL ON FUNCTION "public"."realtime_broadcast_trips"() TO "anon";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_trips"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."realtime_broadcast_trips"() TO "service_role";



GRANT ALL ON FUNCTION "public"."release_stale_agent_job_locks"("p_stale_threshold" interval) TO "anon";
GRANT ALL ON FUNCTION "public"."release_stale_agent_job_locks"("p_stale_threshold" interval) TO "authenticated";
GRANT ALL ON FUNCTION "public"."release_stale_agent_job_locks"("p_stale_threshold" interval) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_agent_job_progress"("p_job_id" "uuid", "p_progress" integer, "p_current_step" "text") TO "anon";
GRANT ALL ON FUNCTION "public"."update_agent_job_progress"("p_job_id" "uuid", "p_progress" integer, "p_current_step" "text") TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_agent_job_progress"("p_job_id" "uuid", "p_progress" integer, "p_current_step" "text") TO "service_role";



GRANT ALL ON FUNCTION "public"."update_collection_count"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_collection_count"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_collection_count"() TO "service_role";
...
GRANT ALL ON TABLE "public"."budget_tracking" TO "anon";
GRANT ALL ON TABLE "public"."budget_tracking" TO "authenticated";
GRANT ALL ON TABLE "public"."budget_tracking" TO "service_role";



GRANT ALL ON TABLE "public"."car_rentals" TO "anon";
GRANT ALL ON TABLE "public"."car_rentals" TO "authenticated";
GRANT ALL ON TABLE "public"."car_rentals" TO "service_role";
...
GRANT ALL ON TABLE "public"."rental_freshness_log" TO "anon";
GRANT ALL ON TABLE "public"."rental_freshness_log" TO "authenticated";
GRANT ALL ON TABLE "public"."rental_freshness_log" TO "service_role";



GRANT ALL ON TABLE "public"."rental_listing_images" TO "anon";
GRANT ALL ON TABLE "public"."rental_listing_images" TO "authenticated";
GRANT ALL ON TABLE "public"."rental_listing_images" TO "service_role";



GRANT ALL ON TABLE "public"."rental_listing_sources" TO "anon";
GRANT ALL ON TABLE "public"."rental_listing_sources" TO "authenticated";
GRANT ALL ON TABLE "public"."rental_listing_sources" TO "service_role";



GRANT ALL ON TABLE "public"."rental_search_sessions" TO "anon";
GRANT ALL ON TABLE "public"."rental_search_sessions" TO "authenticated";
GRANT ALL ON TABLE "public"."rental_search_sessions" TO "service_role";
...
GRANT ALL ON TABLE "public"."tourist_destinations" TO "anon";
GRANT ALL ON TABLE "public"."tourist_destinations" TO "authenticated";
GRANT ALL ON TABLE "public"."tourist_destinations" TO "service_role";
...
CREATE TRIGGER on_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();
  ...
  for insert
  to authenticated
with check (((extension = ANY (ARRAY['broadcast'::text, 'presence'::text])) AND (((realtime.topic() ~~ 'conversation:%'::text) AND (EXISTS ( SELECT 1
   FROM public.conversations c
  WHERE ((c.id = (split_part(realtime.topic(), ':'::text, 2))::uuid) AND (c.user_id = auth.uid()))))) OR ((realtime.topic() ~~ 'trip:%'::text) AND (EXISTS ( SELECT 1
   FROM public.trips t
  WHERE ((t.id = (split_part(realtime.topic(), ':'::text, 2))::uuid) AND (t.user_id = auth.uid()))))))));
  ...
  for select
  to authenticated
using (((extension = ANY (ARRAY['broadcast'::text, 'presence'::text])) AND (((realtime.topic() ~~ 'conversation:%'::text) AND (EXISTS ( SELECT 1
   FROM public.conversations c
  WHERE ((c.id = (split_part(realtime.topic(), ':'::text, 2))::uuid) AND (c.user_id = auth.uid()))))) OR ((realtime.topic() ~~ 'trip:%'::text) AND (EXISTS ( SELECT 1
   FROM public.trips t
  WHERE ((t.id = (split_part(realtime.topic(), ':'::text, 2))::uuid) AND (t.user_id = auth.uid()))))) OR ((realtime.topic() ~~ 'job:%'::text) AND (EXISTS ( SELECT 1
   FROM public.agent_jobs j
  WHERE ((j.id = (split_part(realtime.topic(), ':'::text, 2))::uuid) AND (j.user_id = auth.uid()))))) OR (realtime.topic() ~~ (('user:'::text || (auth.uid())::text) || '%'::text)))));
