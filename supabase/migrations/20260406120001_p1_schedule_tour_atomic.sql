-- P1 CRM atomic RPC: schedule a tour (lead + showing) in one transaction.
-- Split from original 20260406120000 to avoid multi-statement prepared-protocol issues.
-- Same-day conflict detection uses America/Bogota to match idx_showings_lead_apt_day.
...
BEGIN
  ...
  INSERT INTO public.leads (
    user_id, neighborhood_id, source, email, phone, notes, metadata, idempotency_key
  )
  VALUES (
    p_user_id, p_neighborhood_id,
    COALESCE(nullif(trim(p_source), ''), 'web'),
    p_email, p_phone, p_notes,
  ...
  IF NOT FOUND THEN
    BEGIN
      INSERT INTO public.showings (
        lead_id, apartment_id, scheduled_at, status, renter_notes, metadata
      )
      VALUES (
        v_lead.id, p_apartment_id, v_scheduled, 'scheduled',
        p_renter_notes,
    ...
    EXCEPTION
      WHEN unique_violation THEN
        ...
        IF NOT FOUND THEN
          -- America/Bogota match idx_showings_lead_apt_day — session-tz ::date
          -- would mis-bucket rows near midnight Bogota and miss real conflicts.
          IF EXISTS (
  ...
  RETURN jsonb_build_object('lead', to_jsonb(v_lead), 'showing', to_jsonb(v_showing));
END;
$$;
