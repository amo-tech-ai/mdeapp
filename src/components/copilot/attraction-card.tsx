...
function priceLabel(priceUsd?: number): string | null {
  if (priceUsd == null) return null;
  return priceUsd === 0 ? "Free" : `$${priceUsd}`;
}

function AttractionMapLinks({ mapsUrl }: { mapsUrl?: string | null }) {
  ...
  if (!deepLinks) {
    return (
      <a
        ...
        data-testid="attraction-card-maps-link"
  ...
  return (
    <a
      ...
      data-testid="attraction-card-directions-link"
      onClick={(e) => e.stopPropagation()}
    >
...
export function AttractionCard({
  ...
  const typeLabel = categoryLabel(category) ?? "Attraction";
  const entryLabel = priceLabel(priceUsd);
  const durationLabel =
  ...
  const timeLabel =
    bestTimeOfDay && bestTimeOfDay !== "any"
      ? `Best: ${bestTimeOfDay}`
      : null;
  const blurb = aiSummary?.trim() || null;
  ...
  return (
    <article
      ...
      data-testid={testId}
      data-result-kind="attraction"
      data-pin-id={pinId}
      data-selected={selected ? "true" : "false"}
      onMouseEnter={preview}
      onFocus={preview}
      aria-label={`Attraction: ${title}${neighborhood ? `, ${neighborhood}` : ""}`}
    >
      <div
      ...
      >
        {imageUrl ? (
          <div className="shrink-0">
            <div
              ...
              data-testid="attraction-card-photo"
            >
          ...
          <div
            ...
            data-testid="attraction-card-photo-placeholder"
            aria-hidden
          >
            Attraction
          </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            ...
            {ratingText ? (
              <p
                ...
                data-testid="attraction-card-rating"
              >
                ★ {ratingText}
              </p>
            ) : null}
          ...
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            ...
            {entryLabel ? <Badge variant="outline">{entryLabel}</Badge> : null}
            {durationLabel ? (
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t px-3 py-2">
        <AttractionMapLinks mapsUrl={mapsUrl} />
        <div className="flex flex-wrap gap-1.5">
          {onOpenDetails ? (
            <Button
              ...
              size="sm"
              variant="outline"
              data-testid="attraction-details-cta"
              onClick={(e) => {
                e.stopPropagation();
                openDetails();
              }}
