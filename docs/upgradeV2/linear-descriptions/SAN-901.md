...
**Blocked by:** — (gates cleared: [SAN-900](https://linear.app/sanjiovani/issue/SAN-900) · [SAN-895](https://linear.app/sanjiovani/issue/SAN-895) · [SAN-905](https://linear.app/sanjiovani/issue/SAN-905) merged [#216](https://github.com/amo-tech-ai/mdeapp/pull/216) @ `34d6831`)

**Unblocks:** SAN-890 full `/chat` migration directly (SAN-911 parked)
...
De-risk SAN-890 by proving the v2 subpath pattern on `/chat` with **minimal surface area**: `useAgent(conciergeAgent)` + one `useRenderTool` + one `useHumanInTheLoop` — zero console errors on flag-on localhost.
...
| **Sofía** | Spike failure blocks full migration — no SAN-890 until PASS. | Evidence JSON green |
...
#### A. Gate
- [x] **A1** SAN-900 `09-file-map.md` merged — touch order sourced from map
- [x] **A2** SAN-895 + SAN-905 landed — host console hygiene gate passed ([#216](https://github.com/amo-tech-ai/mdeapp/pull/216) @ `34d6831`)
...
- [ ] **E3** If FAIL: document blocker in SAN-890 — do not start full migration
