...
- **Orient before any code (graph → canon → reuse).** For class C/U/S, before editing: (1) **graph** — `scripts/graphify-run.sh query "<thing>" | path "<A>" "<B>" | explain "<symbol>"` to locate; these return BFS *neighbors*, not a complete caller list, so verify callers before a breaking change. (2) **canon** — read the deterministic guardrail for the surface: UI→`DESIGN.MD`, route→`sitemap.md`, CopilotKit/Mastra/Maps/Supabase→`LESSONS.md` (don't rely on the graph to surface these). (3) **reuse** — "does this already exist?" Check the graph **and** the domain `INDEX.md`/`sitemap.md` (graph BFS matches names, so it misses differently-named duplicates — the forked-classifier / duplicate-card drift). **Stale-graph guard:** the graph is a last-build snapshot; if the branch has commits since, run `npm run graphify:update` before trusting a locate. Full spec: [`docs/ai-second-brain/lean-plan.md`](docs/ai-second-brain/lean-plan.md) Part 7. (→ skill `graphify`)
